...
def test_scheduled_material_echo_blocked():
    bad = "# 标题\n\n## 编译规则\n- 复述素材包\n"
    r = server._twin_impl("submit", {"work_type": "周报", "prompt_md": bad,
    ...
    assert server._twin_impl("status", {})["prompts"] == []
...
def test_scheduled_no_headings_blocked():
    r = server._twin_impl("submit", {"work_type": "周报", "prompt_md": "没有标题的纯文本稿",
                               "origin": "scheduled", "source_memory_ids": []})
    assert r["ok"] is False and r["error"] == "validation_failed"
...
def test_g1_selflock_downgrades_when_active_carries_marker():
    _mk_uncompiled([611])
    v1 = "# 风格\n\n## 编译规则（反面示例，勿照抄分区）\n\n- 命名规则\n"
    r1 = server._twin_impl("submit", {"work_type": "周报", "prompt_md": v1, "model": "m"})
    # 交互式首版：echo 是违规但只警告（用户治理优先），落版成功
    assert r1["ok"] and any("素材回声" in w for w in r1["warnings"])
    v2 = "# 风格\n\n## 编译规则（反面示例，勿照抄分区）\n\n- 命名规则（沿袭 v1）\n"
    r2 = server._twin_impl("submit", {"work_type": "周报", "prompt_md": v2,
    ...
    # active 干净 + 新稿带标记 → 照拦
    _mk_uncompiled([612])
    server._twin_impl("submit", {"work_type": "PPT", "prompt_md": "# 干净版\n\n- 规则\n",
                           "model": "m"})
    r3 = server._twin_impl("submit", {"work_type": "PPT",
                                "prompt_md": "# 干净版\n\n## 编译规则\n- 复述\n",
...
def test_g1_level_rewrite_detected():
    r = server._twin_impl("submit", {"work_type": "周报",
                               "prompt_md": "# 标题\n\n# 编译规则\n- 复述\n",
                               "origin": "scheduled", "source_memory_ids": []})
    assert r["ok"] is False
    assert any(v["check"] == "material_echo" for v in r["violations"])
...
def test_g3_warning_alive_semantics():
    ...
    _mk_uncompiled([603])
    r = server._twin_impl("submit", {"work_type": "周报", "prompt_md": GOOD_MD,
...
def test_damping_allows_new_evidence_and_stale():
    ...
    r3 = server._twin_impl("submit", {"work_type": "周报", "prompt_md": GOOD_MD,
                                "origin": "scheduled", "source_memory_ids": []})
    ...
    r2 = server._twin_impl("submit", {"work_type": "周报", "prompt_md": GOOD_MD,
                                "origin": "scheduled", "source_memory_ids": [721, 722]})
...
def test_nightly_reject_clears_on_success():
    server._twin_impl("submit", {"work_type": "周报", "prompt_md": "无标题稿",
                           "origin": "scheduled", "source_memory_ids": []})
    server._twin_impl("submit", {"work_type": "周报", "prompt_md": "仍是无标题稿",
                           "origin": "scheduled", "source_memory_ids": []})
    s = server._twin_impl("status", {})
    assert s["nightly_rejected"][0]["count"] == 2
    # 人工重出成功 → 计数清
    r = server._twin_impl("submit", {"work_type": "周报", "prompt_md": GOOD_MD, "model": "m"})
    assert r["ok"]
    assert flow.get_meta("nightly_reject:work_report") is None
    assert "nightly_rejected" not in server._twin_impl("status", {})
...
def test_material_markers_drift_guard(monkeypatch):
    ...
    m1 = server._twin_impl("compile", {"work_type": "周报"})
    m2 = server._twin_impl("compile", {"work_type": "aud-leadership"})
...
def test_foreign_ids_rejected_for_scheduled():
    _mk_uncompiled([631])
    server._twin_impl("submit", {"work_type": "周报", "prompt_md": GOOD_MD,
                                 "model": "m", "source_memory_ids": [631]})
    _mk_uncompiled([632])
    r = server._twin_impl("submit", {"work_type": "周报", "prompt_md": GOOD_MD,
                                     "origin": "scheduled",
                                     "source_memory_ids": [631, 632, 999]})
    assert r["ok"] is False and r["error"] == "validation_failed"
    assert r["violations"][0]["check"] == "foreign_ids" and "999" in r["violations"][0]["detail"]
    assert server._twin_impl("status", {})["prompts"][0]["active"] == 1  # 未落版


def test_foreign_ids_stripped_for_interactive():
    _mk_uncompiled([641])
    r = server._twin_impl("submit", {"work_type": "周报", "prompt_md": GOOD_MD,
                                     "model": "m", "source_memory_ids": [641, 888, 641]})
    assert r["ok"] and r["source_count"] == 1  # 多余剔除 + 去重
    assert any("已剔除" in w and "888" in w for w in r["warnings"])
    assert any("去重剔除 1 个重复" in w for w in r["warnings"])


def test_g2_accepts_no_space_chinese_heading():
    """评审轮2 P2-3：中文输出「##标题」无空格也认（与 G1 行首口径对齐），不再误拦。"""
    r = server._twin_impl("submit", {"work_type": "周报",
                                     "prompt_md": "#整体风格\n\n- 规则\n",
                                     "origin": "scheduled", "source_memory_ids": []})
    assert r["ok"] is True  # 首版无 active，空转阻尼不适用；标题被认出 → 落版
    assert "no_headings" not in json.dumps(r.get("violations", []))
